...
public class MatchModelTests
{
    ...
    public void Should_Create_Match_Without_Eloquent()
    {
        ...
        CypherBuilder.ToString().NormalizeWhitespace().Should().Be("MATCH (l0:Person)\n".NormalizeWhitespace());
    }
    [Fact]
    public void Should_Create_Match_With_Eloquent()
    {
        ...
        sut.ToCypher(CypherBuilder);
        CypherBuilder.ToString().NormalizeWhitespace().Should().Be("MATCH (l0:Person WHERE l0.Id = $qp_0_0)\n".NormalizeWhitespace());
    }
    [Fact]
    public void Should_Create_Match_OutGoing_Relation_Without_Eloquent()
    {
        ...
        sut.ToCypher(CypherBuilder);
        CypherBuilder.ToString().NormalizeWhitespace().Should().Be("MATCH (l0)-[r1:ACTED_IN]->(l1:Movie)\n".NormalizeWhitespace());
    }
    [Fact]
    public void Should_Create_Match_OutGoing_Relation_With_Eloquent()
    {
        ...
        sut.ToCypher(CypherBuilder);
        CypherBuilder.ToString().NormalizeWhitespace().Should().Be("MATCH (l0)-[r1:ACTED_IN]->(l1:Movie WHERE l1.Id = $qp_1_0)\n".NormalizeWhitespace());
    }
    [Fact]
    public void Should_Create_Match_InComing_Relation_Without_Eloquent()
    {
        ...
        sut.ToCypher(CypherBuilder);
        CypherBuilder.ToString().NormalizeWhitespace().Should().Be("MATCH (l0)<-[r1:ACTED_IN]-(l1:Person)\n".NormalizeWhitespace());
    }
    [Fact]
    public void Should_Create_Match_InComing_Relation_With_Eloquent()
    {
        ...
        sut.ToCypher(CypherBuilder);
        CypherBuilder.ToString().NormalizeWhitespace().Should().Be("MATCH (l0)<-[r1:ACTED_IN]-(l1:Person WHERE l1.Id = $qp_1_0)\n".NormalizeWhitespace());
    }


    [Fact]
    public void Should_Create_Optional_Match_Relation_Without_Eloquent()
    {
        var firstMatch = new MatchModel<Movie>(null, 0);
        var relationConfig = new RelationConfiguration<Movie, Person>("ACTED_IN", RelationDirection.In);
        var sut = new MatchRelationModel<Person>(firstMatch, relationConfig, null, 1) { Optional = true };
        sut.ToCypher(CypherBuilder);
        CypherBuilder.ToString().NormalizeWhitespace().Should().Be("OPTIONAL MATCH (l0)<-[r1:ACTED_IN]-(l1:Person)\n".NormalizeWhitespace());
    }
    [Fact]
    public void Should_Create_Optional_Match_InComing_Relation_With_Eloquent()
    {
        var firstMatch = new MatchModel<Movie>(null, 0);
        var eloquent = new Eloquent<Person>(1);
        eloquent.Where(x => x.Id, Guid.NewGuid());
        var relationConfig = new RelationConfiguration<Person, Person>("ACTED_IN", RelationDirection.In);
        var sut = new MatchRelationModel<Person>(firstMatch, relationConfig, eloquent, 1) { Optional = true };
        sut.ToCypher(CypherBuilder);
        CypherBuilder.ToString().NormalizeWhitespace().Should().Be("OPTIONAL MATCH (l0)<-[r1:ACTED_IN]-(l1:Person WHERE l1.Id = $qp_1_0)\n".NormalizeWhitespace());
    }
}